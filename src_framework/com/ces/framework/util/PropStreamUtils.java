 ...
 * 
 * 	通过流读取，获取的不是最新值，可以避免path读取找不到文件的问题(例如：***.jar!\config.properties)
 * 
 * @author hc
...
public class PropStreamUtils {
	...
	public static String getStringValue(String filePath, String key) {
		...
		try {
			...
			InputStream in = PropStreamUtils.class.getClassLoader()
	...
	public static void writeProp(String filePath, String key, String value) {
		...
		try {
			ins = PropStreamUtils.class.getClassLoader()
					.getResourceAsStream(filePath);
			prop.load(ins);
			ins.close();

			URL url = PropStreamUtils.class.getClassLoader().getResource(filePath);
